...
def test_generate_daily_picks_warns_if_pinnacle_enabled_but_no_football_events_counted(caplog):
    """Guarda de regresión del bug real de 2026-09-07: con
    pinnacle_reference.enabled=true, si `events` tiene partidos pero NINGUNO
    cuenta como Event.sport == "football" (como pasaba antes por un valor
    mal poblado), debe quedar un WARNING explícito en el log — antes esto
    era completamente silencioso (solo se notaba leyendo con cuidado el
    "0/0" del log INFO), y la integración de Pinnacle nunca se activaba sin
    que nadie se enterara."""
    with tempfile.TemporaryDirectory() as tmp:
        storage = Storage(str(Path(tmp) / "t.db"))
        cfg = _pinnacle_cfg(enabled=True)
        provider = _FakeProviderForPinnacleReference(include_bet365=True)
        # Simula el bug real: el evento que llega desde el provider no tiene
        # sport == "football" (como pasaba cuando se parseaba mal desde el
        # JSON crudo de odds-api.io).
        provider._event.sport = "otra-cosa"

        with patch("valuebet.pinnapi_provider.PinnapiProvider") as MockProvider:
            MockProvider.return_value.get_soccer_events.return_value = []
            with caplog.at_level("WARNING", logger="valuebet.daily"):
                generate_daily_picks(cfg, provider, storage)

    assert any(
        "Event.sport == 'football'" in record.message or "no puede emparejar" in record.message
